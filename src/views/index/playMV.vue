<template>
    <div class="container">
      ...
      <div class="detail" v-if="isfetchData">
        <p class="mv-name"> {{ MVDetail.name }}</p><!-- MV名字: -->
        <p class="mv-desc">简介: {{ MVDetail.desc }}</p><!-- MV简介: -->
        
        <img
          :src="MVDetail.artists[0].img1v1Url"
          :title="MVDetail.artists[0].name"
          alt=""
          class="artist-image"
        />
        <p class="mv-artist">歌手: {{ MVDetail.artists[0].name }}</p>
        <p class="mv-publish">发布年份: {{ MVDetail.publishTime }}</p>
      </div>
...
import { getMVDetail,getMVurl } from '../../apis/http';
...
const fetchData = async () =>{
    try{
        const [res, ress] = await Promise.all([getMVDetail(MVId), getMVurl(MVId)]);
        MVDetail.value = res.data.data;
        MVurl.value = ress.data.data;
        console.log("MVDetail.value",MVDetail.value);
        console.log("MVurl.value",MVurl.value);
        options.title=MVDetail.value.name;
        options.src=MVurl.value.url;
        options.poster=MVDetail.value.cover;
...
const options = reactive({
  ...
  height: "40%", //播放器高度
  color: "#409eff", //主题色
  title: 'MVDetail.value.name', //视频名称
  src: 'MVurl.value.url', //视频源
  poster: 'MVDetail.value.cover', //封面
  muted: false, //静音
  webFullScreen: false,
